...
(0.8.0) and backup/export (0.10.0), and deliberately *before* rate limiting and
learning mode.
...
one place is EasyLog's job (0.8.0), which is the right split: aggregating logs
...
log (0.8.0) with what changed and which peer received it.
