...
(0.11.0) and backup/export (0.13.0), and deliberately *before* rate limiting and
learning mode.
...
one place is EasyLog's job (0.11.0), which is the right split: aggregating logs
...
log (0.11.0) with what changed and which peer received it.
...
* **Does the replica's GUI allow local override?** Recommendation: no, with one
  exception worth considering — the IP allow/block list, since 0.9.0's whole
