...
(0.9.0) and backup/export (0.11.0), and deliberately *before* rate limiting and
learning mode.
...
## It is 0.10.0's export, applied continuously
...
* **Does the replica's GUI allow local override?** Recommendation: no, with one
  exception worth considering — the IP allow/block list, since 0.13.0's whole
