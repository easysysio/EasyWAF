...
Status: planned for **0.10.0** — see [roadmap.md](roadmap.md), after rule
updates (0.6.0), flow logs (0.8.0) and load balancing (0.9.0).
...
features with different failure modes. Pick one for 0.10.0 and say which; a
...
before 0.10.0, **the snapshot half can be pulled forward on its own** — it is
