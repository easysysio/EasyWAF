...
* **No backup, restore or configuration export.** Everything lives in one SQLite file with no
  ...
  **0.12.0**; until then, copy the database file with the service stopped.
* **No high availability.** No configuration sync between nodes. Scheduled for **0.13.0**.
* **No IP allow/block lists** (**0.9.0**) and **no rate limiting** (**0.14.0**).
