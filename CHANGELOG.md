...
- A site is created with its whole pool: the Upstream field takes one backend or several, one per line, each with an optional weight after the URL (`http://10.0.0.8:3000 3`), and session affinity is a checkbox on the same form — nothing about a pool waits until the site exists. More backends can be added the same way afterwards. A site's backends are weighted, switched off and removed under **Upstreams** on its page, which also shows what this node has observed of each one — in rotation, failing, or out and when it will be tried again. A site can never be left with nothing to forward to.
